...
class square_diff_thr: public thrust::unary_function<thrust::tuple<T,T>,T> {
    ...
    T operator()(const thrust::tuple<T,T>& x) const {
        return (thrust::get<1>(x) - thrust::get<0>(x)) * (thrust::get<1>(x) - thrust::get<0>(x)); 
    }
};
...
T diffGPU(T *A_d, T *B_d, int N) {
    ...
    ZipIterator first = thrust::make_zip_iterator(thurst::make_tuple(A_ptr,B_ptr));
    ZipIterator last = thrust::make_zip_iterator(thrust::make_tuple(A_ptr + N*N, B_ptr + N*N));
    T a1 = thrust::transform_reduce(first,last,square_diff_thr<T>(), static_cast<T>(0), thrust::plus<T>());
    T a2 = thrust::transform_reduce(B_ptr, B_ptr + N*N, square_thr<T>(), static_cast<T>(0), thrust::plus<T>());
    return sqrt(a1/a2);
}
